...
An interactive 360° virtual tour built with A-Frame. It features smooth progressive image loading, tap/click hotspots for navigation, touch gestures (drag + pinch), mouse zoom, and performance-friendly prefetching.
...
> [!TIP]
> Quick check for hotspot `position`
>  1. In DevTools Console, adjust the hotspot to where you want:
>     ```js
>     HOTSPOT_ID.setAttribute('position', '-1 -1 -1')
>     ```
>  2. Copy the final `position` value to the hotspot.
...
- Requirements
  ...
  - A-Frame 1.5.0 is loaded via CDN.
